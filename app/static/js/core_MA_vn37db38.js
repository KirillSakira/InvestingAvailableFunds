        ...
        try {
          ...
          if (res["status"] == "success") {
            ...
            let html = cloneClientBlock
              ...
              .replace(
                "$color",
                toString(res.balance_proc).includes("-") ? "col_red" : "col_lb"
              );
...
if (pathname[1] == "trade") {
  $(".trade_tabs").on("click", ".el_hoverer", function () {
    window.open(
      "/securitiesTrade/" + pathname[2] + "/" + this.getAttribute("data-id"),
      "_self"
    );
  });
...
function resizeHe() {
  if (["operations", "tradeHistory"].indexOf(pathname[1]) !== -1) {
    $(".nomh > div > div:nth-child(2)").css(
      "max-height",
      "calc(100vh - " +
        ($(".header").height() +
          $("nav").height() +
          $(".nomh > div > div.h1").height()) +
        "px - 11.5rem)"
    );
  }
  if (pathname[1] == "trade") {
...
function line_graph_view(timeKey) {
  generLine("lineChart", timeKey, graph_line);
}
