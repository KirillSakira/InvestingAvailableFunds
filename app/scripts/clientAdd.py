...
from app.scripts.funcs import check, returnJson
...
def clientAdd(request):
	...
	dataBase.execute(f'select id_user from auth_user where login={username}')
	idUserEnt = dataBase.fetchall[0][0]
	if idUserEnt == None:
		return returnJson(status='error', message='Такого пользователя не существует')
	dataBase.execute(f'select id_ent from users where id_user={idUserEnt}')
