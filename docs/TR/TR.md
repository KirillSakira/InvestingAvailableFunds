# В данной версии тз все схемы и диаграммы являются устаревшими, что вдальнейшем будет исправлено
...
1. Главная страница
    - список из заявок (ФИО клиента, размер пополнения/снятия) (при отправке пользователем формы, она попадает в общую
        для всех заявок от всех пользователей очередь из заявок, очередь из заявок видна всем сотрудникам, заявку можно
        принять в работу, тогда она привязывается к конкретному сотруднику, принявшему ее и исчезает из общего списка
        заявок)
    - список заявок в работе у конкретного сотрудника
2. Список пользователей, с возможностью посмотреть:
   - каталог приобретенных активов для пользователя
   - размер депозита
   - историю операций с депозитом
   - окно с возможностью отправить сообщение пользователю
   - возможность удалить аккаунт пользователя
   - торговля активами 
3. Личный кабинет 
...
### 6.2. Выбор клиентского аккаунта
В середине главной страницы должен располагаться список из клиентов. 

### 6.3. Главная страница клиента
Основано на 5.1.
...
1. Главная страница
   - Список пользователей, с возможностью посмотреть:
   - Список сотрудников:
   - Список заявок
...
1. Список всех пользователей с возможностью:
    - каталог приобретенных активов для пользователя
    - размер депозита
    - историю операций с депозитом
    - окно с возможностью отправить сообщение пользователю
...
2. Список всех сотрудников:
    - окно с возможностью отправить сообщение сотруднику
    - возможность удалить аккаунт сотрудника
    - посмотреть активные заявки сотрудника
3. Список всех заявок
4. Возможность открыть личный кабинет

### 7.2. Личный кабинет админа

Аналогичен лк сотрудника, кроме изменения TID, cv пункт 7.6

### 7.3. Личный кабинет

Абсолютно аналогично пользователю, однако есть возможность редактирования информации и удаления его аккаунта

### 7.4. Каталог приобретенных активов

Список из имеющихся у клиента активов, реализованных ввиде списка из блоков для каждого вида активов. Данный список
будет иметь структуру близкую с каталогом при приобретении сотрудником активов. Для наглядности необходим дизайнер.

### 7.5. Обратная связь

Возможность заполнить форму для любого из ползователей из списка на главной страице

### 7.6. Создание аккаунта сотрудника
В личном кабинете есть некоторая константа TID типа: `sajzyv-6jasFo-vyxjom`, которую необходимо будет ввести новому 
сотруднику при регистрации, это константа, она одинакова для всех новых сотрудников и требуется раз при создании 
рабочего аккаунта. Админ может перегенерировать эту константу в ЛК, тогда всем новым сотрудникам потребуется при 
регистрации использовать новую константу.
